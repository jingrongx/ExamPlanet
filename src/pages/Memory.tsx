...
export function Memory() {
  ...
  return (
    <div className="space-y-4">
      ...
      {mode === 'browse' ? (
        <BrowseMode questions={questions} licenseColor={license.color} />
      ) : (
...
// 口诀浏览模式
function BrowseMode({ questions, licenseColor }: { questions: Question[]; licenseColor: string }) {
  const toast = useToast()
  ...
  return (
    <div className="space-y-2">
      <p className="text-[11px] text-stardust/50 px-1">点击题目展开 AI 生成的多种记忆口诀</p>
      {questions.map((q, i) => {
              ...
              <div className="flex-1 min-w-0">
                ...
                {builtin && (
                  <div className="mt-1 flex items-center gap-1">
                    <span className="text-[9px] px-1.5 py-0.5 rounded bg-neon-gold/20 text-neon-gold">⭐ 内置口诀</span>
                  </div>
                )}
              </div>
            ...
            <AnimatePresence>
              {isOpen && (
                ...
                >
                  ...
                  {generated[q.id]?.map((m, j) => (
                    <div key={j} className="glass p-3 rounded-xl" style={{ borderLeft: '2px solid #00f5ff' }}>
                      <p className="text-xs text-stardust/90 leading-relaxed">{m}</p>
                    </div>
                  ))}
                  <div className="flex items-center gap-2 pt-1">
                    <button
                      onClick={() => speak(q.question + ' 答案是 ' + q.answer)}
